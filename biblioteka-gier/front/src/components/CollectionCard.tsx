...
function CollectionCard(props: { [key: string | number]: string | number }) {
  ...
  return (
    <>
      <div className="collectionCard" onClick={() => handleClick(Number(props.id))}>
        <h2 className="collectionCard--title">{props.title}</h2>

        <div
          className="collectionCard--content"
          style={props.src ? { background: `url(${props.src})` } : { background: `#444` }}>
          <div className="collectionCard--content-total">
            Total games <br />
            <p className="collectionCard--content-number">{props.value}</p>
          </div>
        </div>
